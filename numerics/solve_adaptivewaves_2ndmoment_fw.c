...
int parsecommandline(int argn, char *argv[]) {
  char c;
  while((c = getopt(argn, argv,"UCi:o:u:c:I:S:a:s:d:z:m:M:v:D:A:QqF:")) != -1){
    switch(c) {
      case 'C':	if(switchu == 1) {
      ...
      case 'q':	verbosity = 0;
		break;
      case 'F':	fisherwavestep = atof(optarg);
		useselectiongradient = 0;
    }
  }
...
void initial_c_approx() {
  ...
  if(useselectiongradient == 1) {
    ...
    for(i=0;i<space;i++) {
      for(j=0;j<=i;j++) {
	c2[i][j] = (i<=space0)*(j<=space0); // hack for getting step function. C uses value '1' (int) for 'true' (boolean).
					    // correct height of profile will be rescaled at beginning of iterations
      }
    }
  }
}
...
int iterate_c_diffusion(int timestep) {
  ...
  for(i=1;i<space-1;i++) {
    ...
    for(j=i;j<space;j++)c[i] += c2[j][i]*u[j];
				// finalize integration from c2 to c1
    c[i] *= dx;
  ...
  for(i=0;i<space-1;i++)memcpy(c2[i],c2new[i],(i+1)*sizeof(double));
}
...
void print_options() {
  ...
  printf("#\n");
  if(mutation_type == 1) {
    printf("# mutationmodel     = diffusion\n");
  }else if(mutation_type > 1) {
    printf("# mutationmodel     = jumps\n");
    printf("# jumpwidth         = %d\n",mutation_jumpwidth);
  }
...
// ===========================================================================================
// adjust the density to fulfill the constraint \int dx dy u(x) u(y) c2(x,y) = 1
// ===========================================================================================
...
void get_g_peak(int step) {
  ...
  xg *= dx*dx;
  popsize *= dx;
  printf("%10d\t%17.10e\t%17.10e\t%17.10e\n",step,xc,xg,popsize);
}
...
void print_coeff() {
  ...
  for(i=0;i<space;i++) {
    for(j=0;j<space;j++) {
      printf("%e %e %e\n",(i-space0)*dx,(j-space0)*dx,c_coeff_0[i]+c_coeff_0[j]);
    }
    printf("\n");
  }
}
