...
int parsecommandline(int argn, char *argv[]) {
  char c;
  while((c = getopt(argn, argv,"UCi:o:u:I:S:a:s:d:z:m:M:v:D:Qq")) != -1){
    switch(c) {
      case 'C':	if(switchu == 1) {
...
int iterate_u_diffusion(int timestep) {
  ...
  // u[-1] = 0
  f = u_coeff_m[0] * u[1] + u_coeff_0[0] * u[0] - 2.*u[0]*u[0];
  ...
  // u[space] = x/2
  f = u_coeff_m[space-1] * u[space-2] + u_coeff_0[space-1]*u[space-1] + u_coeff_p[space-1] * (space-space0)*dx*0.5 - 2.*u[space-1]*u[space-1];
...
int iterate_u_jumps(int timestep) {
  ...
  for(i=mutation_jumpwidth;i<space-mutation_jumpwidth;i++) {
    f = mutation_asymmetry*u_coeff_j[i]*u[i-mutation_jumpwidth] + u_coeff_m[i]*u[i-1] + u_coeff_0[i]*u[i] + u_coeff_p[i]*u[i+1] + u_coeff_j[i]*u[i+mutation_jumpwidth] - 2.*u[i]*u[i]; 
    fu = u_coeff_0[i] - 4.*u[i];
    unew[i] = u[i] - alpha*f/fu;
  }
  for(i=space-mutation_jumpwidth;i<space-1;i++) {
    f = mutation_asymmetry*u_coeff_j[i]*u[i-mutation_jumpwidth] + u_coeff_m[i]*u[i-1] + u_coeff_0[i]*u[i] + u_coeff_p[i]*u[i+1] + (i-space0)*dx*0.5*u[i+mutation_jumpwidth] - 2.*u[i]*u[i]; 
    fu = u_coeff_0[i] - 4.*u[i];
    unew[i] = u[i] - alpha*f/fu;
  }
...
void initialize() {
  ...
  if (switchu == 1) {
	...
	case 1:	u_coeff_m[i] += mutation_rate/(dx*dx);
		...
		break;
	case 2:	u_coeff_0[i] += -mutation_rate*(1+mutation_asymmetry);
