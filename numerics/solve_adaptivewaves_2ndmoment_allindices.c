...
void read_c2_file(int extractparameters) {
  ...
  if(fpc != NULL) {
    ...
    for(i=0;i<space;i++) {
      ...
      fread(&c2[i][0],space,sizeof(double),fpc);
    }
    free(tmpd);
    fclose(fpc);
  }
...
void write_c2_file() {
  ...
  for(i=0;i<space;i++) {
    fwrite(&c2[i][0],space,sizeof(double),fpc);
  }
  fclose(fpc);
}
...
//     c[i] *= dx;
    ...
    for(j=1;j<space-1;j++) {
      ...
      if(i==j) {
	f  += 2.*c[i];
	fc += 2.*u[i];// *dx; Kronecker-Delta = 1/dx * delta_ij
      }
  ...
  for(i=0;i<space;i++)memcpy(&c2[i][0],&c2new[i][0],space*sizeof(double));
}
...
void initialize() {
  ...
  if (switchc == 1) {
    ...
    for(i=0;i<space;i++)c2new[i] = (double*)malloc(space*sizeof(double));
...
void adjust_constraint() {
  ...
  for(i=1;i<space-1;i++) {
    for(j=1;j<space-1;j++) {
      sum += u[i]*u[j]*c2[i][j];	// calculate offdiagonal elements
    }
  }
...
void get_g_peak(int step) {
  int i;
  double xg = 0.,xc = 0.,popsize = 0.,xxc = 0.,xxg = 0.;
  double x;
  for(i=0;i<space;i++) {
    x = (i-space0)*dx;
    xg += x*u[i]*c[i];
  ...
  xxc /= popsize;
  popsize *= dx;
  printf("%10d\t%17.10e\t%17.10e\t%17.10e\t%17.10e\t%17.10e\n",step,xc,xxc-xc*xc,xg,xxg-xg*xg,popsize);
}
  
void write_integrated_c1(int step) {
  int icount=0,dcount=0;
  FILE *fpc1;
  char fname_c1[128];
  
  sprintf(fname_c1,"c1_intermediate_%010d.conf",step);
  fpc1 = fopen(fname_c1,"wb");
  fwrite(&icount,1,sizeof(int),fpc1);
  fwrite(&dcount,1,sizeof(int),fpc1);
  
  fwrite(&dx,1,sizeof(double),fpc1);
  fwrite(&space,1,sizeof(int),fpc1);
  fwrite(&space0,1,sizeof(int),fpc1);
  
  fwrite(&c[0],space,sizeof(double),fpc1);
