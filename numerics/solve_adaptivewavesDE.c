...
// mutations
int    mutation_type = 3; // 1 - diffusion, 2 - jumps, 3 - expdecay, 4 - doubleexponential, 5 - external file
double mutation_rate = 1e-5;
int    mutation_jumpwidth = 20;
char   mutation_kernelfile[128];
double *mutation_kernel;
double mutation_expdecay_p = 0.001;
double mutation_expdecay_n = 0.001;
double mutation_ratiodelmut = 0.5;
...
int parsecommandline(int argn, char *argv[]) {
  char c;
  while((c = getopt(argn, argv,"UCi:o:u:I:S:a:s:d:z:m:M:v:D:QqA:fk:K:")) != -1){
    switch(c) {
      ...
      case 'M':	if(allowmutationoptions == 1) {
		  switch(mutation_type) {
		    ...
		    case 2:	mutation_jumpwidth = atoi(optarg);
				break;
		    case 3:	mutation_expdecay_p = atof(optarg);
				break;
		...
		break;
      case 'k':	mutation_ratiodelmut = atof(optarg);
		break;
      case 'K':	mutation_expdecay_n = atof(optarg);
		break;
...
void initial_u_approx() {
  ...
  double x;
  if(mutation_type == 3) {
    for(i=0;i<=space0;i++)u[i] = 0.5*dx*gsl_sf_exp((i-space0)*dx/mutation_expdecay_p);
  }else{
...
void read_u_file(int extractparameters) {
  ...
  if(fpu != NULL) {
    ...
    if(dcount > 0) {
      ...
      if((dcount >= 5)&&(extractparameters==1)) {
	mutation_rate = tmpd[0];
	wavespeed = tmpd[1];
	mutation_expdecay_p = tmpd[2];
	mutation_expdecay_n = tmpd[3];
	mutation_ratiodelmut = tmpd[4];
      }
      free(tmpd);
    }
...
void write_u_file() {
  ...
  fwrite(&mutation_expdecay_p,1,sizeof(double),fpu);
...
void write_c_file() {
  ...
  if(fpc != NULL) {
    ...
    fwrite(&mutation_expdecay_p,1,sizeof(double),fpc);
    fwrite(&mutation_expdecay_n,1,sizeof(double),fpc);
    fwrite(&mutation_ratiodelmut,1,sizeof(double),fpc);
...
int iterate_c (int timestep) {
  ...
  for (i=2; i<space-2; i+=2) {	/* first even-numbered items */
    dc3 = (c[i+2] - 2.*c[i+1] + 2.*c[i-1] - c[i-2])/(2.*dx*dx*dx);
    dc2 = (c[i+1]-2.*c[i]+c[i-1])/(dx*dx);
    dc1 = (c[i+1]-c[i-1])/(2*dx);
    
    f[i] = a3[i] * dc3 + a2[i] * dc2 + a1[i] * dc1 + a0[i] * c[i];
    fc[i] = -a2[i]*2./(dx*dx) + a0[i];
...
void initialize() {
  ...
  if(switchc == 1) {
    ...
    du1 = (double*)malloc(space*sizeof(double));
    du2 = (double*)malloc(space*sizeof(double));
    
    a3 =  (double*)malloc(space*sizeof(double));
    a2 =  (double*)malloc(space*sizeof(double));
    a1 =  (double*)malloc(space*sizeof(double));
    a0 =  (double*)malloc(space*sizeof(double));
    ...
    for(i=1;i<space-1;i++) {
      s = (i-space0)*dx - 2*u[i] - mutation_rate;
      du1[i] = (u[i+1]-u[i-1])/(2.*dx);
      du2[i] = (u[i+1]-2.*u[i]+u[i-1])/(dx*dx);
      
      a3[i] = -(mutation_expdecay_n*mutation_expdecay_p*wavespeed);
      a2[i] = -((-mutation_expdecay_p + mutation_expdecay_n)*wavespeed + mutation_expdecay_n*mutation_expdecay_p*s);
      a1[i] = -(-wavespeed - (mutation_expdecay_p-mutation_expdecay_p) * s +2.*mutation_expdecay_n*mutation_expdecay_p*(1.-2*du1[i]) + mutation_rate * mutation_expdecay_p * (1.-2.*mutation_ratiodelmut)/(mutation_expdecay_p*(1.-mutation_ratiodelmut) + mutation_ratiodelmut * mutation_expdecay_n));
      a0[i] = -(-s -(mutation_expdecay_p-mutation_expdecay_n)*(1.-2.*du1[i]) + mutation_expdecay_n*mutation_expdecay_p * du2[i] - mutation_rate);
      
      fprintf(stderr,"%lf %e %e %e %e %e %e %e %e\n",(i-space0)*dx,a3[i],a2[i],a1[i],a0[i],u[i],du1[i],du2[i],-a2[i]*2./(dx*dx) + a0[i]);
    }
  }
  exit(1);
}
