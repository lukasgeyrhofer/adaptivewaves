...
int parsecommandline(int argn, char *argv[]) {
  char c;
  while((c = getopt(argn, argv,"UCi:o:u:I:S:a:s:d:z:m:M:v:D:A:P:")) != -1){
    switch(c) {
      case 'C':	if(switchu == 1) {
...
int iterate_u_diffusion(int timestep) {
  ...
  f = (u_coeff_m[0] + u_coeff_p[0]) * u[1] + u_coeff_0[0] * u[0] - u2_coeff[0]*u[0]*u[0];
  fu = u_coeff_0[0] - uder_coeff_u[0]*u[0];
  unew[0] = u[0] - alpha * f/fu;
  for(i=1;i<space-1;i++) {
    f = u_coeff_m[i]*u[i-1] + u_coeff_0[i]*u[i] + u_coeff_p[i]*u[i+1] - u2_coeff[i]*u[i]*u[i];
    fu = u_coeff_0[i] - uder_coeff_u[i]*u[i];
    unew[i] = u[i] - alpha * f/fu;
  }
  f = (u_coeff_m[space-1] + u_coeff_p[space-1]) * u[space-2] + u_coeff_0[space-1]*u[space-1] - u2_coeff[space-1]*u[space-1]*u[space-1];
  fu = u_coeff_0[space-1] - uder_coeff_u[space-1]*u[space-1];
...
//   fprintf(stderr,"#/*jumps*/!!\n");
  ...
  unew[space-1] = u[space-1] - alpha * f/fu;
//   if(unew[space-1]<0)unew[space-1]=0.;
...
void initialize() {
  ...
  if (switchu == 1) {
    ...
    uder_coeff_u = (double*)malloc(space*sizeof(double));
    ...
    for(i=0;i<space;i++) {
      ...
      if(x>-1.) {
	u2_coeff[i] = uprefactor+x;
	uder_coeff_u[i] = 2.*(uprefactor+x);
      }else{
	u2_coeff[i] = uprefactor-1.;
	uder_coeff_u[i] = uprefactor;
      }
...
//       printf("%d\n",i);
    ...
    for(i=1;i<=maxsteps;i++) {
      iterate_c(i);
      if(i%adjuststep == 0)adjust_constraint();
    }
