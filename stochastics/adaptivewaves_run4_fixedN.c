...
void parsecommandline(int argn, char* argv[]) {
  char c;
  int haveinfile = 0;
  while((c = getopt(argn,argv,"c:C:N:e:S:O:R:t:M:qQ")) != -1) {
    switch(c) {
...
void write_output(int step) {
  int i;
  if(quiet < 2) {
    fprintf(stderr,"%10.4lf %16.3lf %16.10lf %16.10lf %16.10lf %16.10lf\n",epsilon*step+starttime,dens_mom0,dens_mom1/dens_mom0,dens_mom2/dens_mom0,lambda,driftvelocity);
    if(quiet < 1) {
      for(i=0;i<space;i++) {
