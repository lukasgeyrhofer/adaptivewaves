...
void write_data() {
  ...
  int i;
  double *dens_write;
  dens_write = (double*)malloc(space*sizeof(double));
  for(i=0;i<space;i++)dens_write[i] = n[i]/dx;
  ...
  fwrite(&n[0],space,sizeof(double),fp);
...
int read_data() {
  ...
  fread(&n[0],space,sizeof(double),fp);
  for(i=0;i<space;i++)n[i]*=dx;
...
double get_mean_pos(double popsize) {
  ...
  double meanpos = 0.0;
  for(i=0;i<space;i++) {
    meanpos += (i-space0)*n[i];
  }
  return meanpos*dx/popsize;
}
...
//   
  ...
  for(i=1;i<space-1;i++) {
    n_new[i]  = n[i] * (determchangefactor + xpos_eps[i]);
    n_new[i] += (n[i+1]+n[i-1])*epsdx2i;
    n_new[i] += (n[i+1]-n[i-1])*vepshalfdxi;
    dw = gsl_ran_poisson(rg,n_new[i])-n_new[i];
    n_new[i] += eps_sqrt_2*dw;
  }
  n_new[last] = 0.;
