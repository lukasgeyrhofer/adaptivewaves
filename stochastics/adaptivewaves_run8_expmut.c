...
void parsecommandline(int argn, char* argv[]) {
  char c;
  int haveinfile = 0;
  while((c = getopt(argn,argv,"c:C:u:e:S:O:R:t:M:qQv:V:E:")) != -1) {
    switch(c) {
      ...
      case 'V':	covar_outputstep = atoi(optarg);
		break;
      case 'E':	mutation_expdecay = atof(optarg);
		break;
    }
  }
...
void reproduction() {
  ...
  for(i=1;i<space-1;i++) {
    ...
    for(j=0;j<i;j++)n_new[i] += mutation_prefactor*exp((j-i)*dx/mutation_expdecay)*n[j];
...
void initialize() {
  ...
  coeff_m[0] = 0.;
  coeff_0[0] = 1. + (0-space0)*dx*epsilon - rho*epsilon;
  coeff_p[0] = epsilon*diffusionconstant/(dx*dx) + driftvelocity*epsilon/dx;
  zeros[0]   = 0.;
  
  for(i=0;i<space;i++) mutation_outflow += epsilon*exp(-i*dx/mutation_expdecay)/mutation_expdecay*diffusionconstant*dx;
  mutation_prefactor = epsilon*diffusionconstant*dx;
  for(i=1;i<space-1;i++) {
    coeff_m[i] =  - 0.5*driftvelocity*epsilon/dx;
    coeff_0[i] = 1. - mutation_outflow + (i-space0)*dx*epsilon - rho*epsilon;
    coeff_p[i] =  + 0.5*driftvelocity*epsilon/dx;
    zeros[i]   = 0.;
  }
  coeff_m[space-1] = - driftvelocity*epsilon/dx;
  coeff_0[space-1] = 1. - mutation_outflow + (space-1-space0)*dx*epsilon - rho*epsilon;
  coeff_p[space-1] = 0.;
  zeros[space-1]   = 0.;
