...
double *u;
double *coeff_m,*coeff_0,*coeff_p,*coeff_j;
...
int mutation_type = 1;
int mutation_jumpwidth = 30;

// function pointers

typedef void (*reprod_func)();
reprod_func reproduction;
...
void read_c() {
  ...
  // all double parameters: for adaptive waves:
  //  * rho (not used here, as rho is dynamically adapted each timestep to center the population)
  //  * starttime
  //  * diffusionconstant =mutationrate*s^2/2 in type1 mutations (diffusion), =mutationrate in type2 mutations (jumps)
  //  * driftvelocity (only u* model)
  ...
  if(icount > 0) {
    ...
    if(icount == 3) {
      fread(&mutation_type,1,sizeof(int),fp);
      fread(&mutation_jumpwidth,1,sizeof(int),fp);
      mutation_jumpwidth = 1;
    }else if(icount>1){
      tmpi=(int*)malloc((icount-1)*sizeof(int));
      fread(&tmpi[0],icount-1,sizeof(int),fp);
      free(tmpi);
    }
  }
...
void write_c() {
  ...
  if(mutation_type == 2) {
    fwrite(&mutation_type,1,sizeof(int),fp);
    fwrite(&mutation_jumpwidth,1,sizeof(int),fp);
  }
...
void reproduction_jumps() {
  int i;
  int last = space-1;
  double rhoeps = rho*epsilon;
//   fprintf(stderr,"mutationjumpwidth=%d\n",mutation_jumpwidth);
  
  n_new[0] = 0.;
  for(i=0;i<mutation_jumpwidth;i++) { // mutational input will be neglected for first few bins, they are not populated anyway (c ~ 10^-100 and lower)
    n_new[i] = (coeff_0[i]-rhoeps) * n[i];
    n_new[i] += twoepssqrt*(gsl_ran_poisson(rg,n[i])-n[i]);
  }
  for(i=mutation_jumpwidth;i<space-1;i++) {
    n_new[i] = (coeff_0[i] - rhoeps)*n[i] + coeff_j[i]*n[i-mutation_jumpwidth];
    n_new[i] += twoepssqrt*(gsl_ran_poisson(rg,n[i])-n[i]);
  }
  n_new[last] = 0.;
...
//   read_u();
  ...
  if(mutation_type == 1) {
    ...
    coeff_p[space-1] = 0.;
    reproduction = reproduction_diffusion;
  }else if(mutation_type == 2) {
    coeff_0[0] = 1. - epsilon*diffusionconstant + (0-space0)*dx*epsilon;
    coeff_j[0] = epsilon*diffusionconstant;
    for(i=1;i<space-1;i++) {
      coeff_0[i] = 1. - epsilon*diffusionconstant + (i-space0)*dx*epsilon;
      coeff_j[i] = epsilon*diffusionconstant;
    }
    coeff_0[space-1] = 1. - epsilon*diffusionconstant + (space-1-space0)*dx*epsilon;
    coeff_j[space-1] = epsilon*diffusionconstant;
    reproduction = reproduction_jumps;
//     print_error("mutation: jumps");
  }else{
    print_error("mutation type not yet implemented");
  }
...
void write_output(int step) {
  ...
  if(quiet < 2) {
    ...
    fprintf(stdout,"%10.3lf %17.10e %17.10e %17.10e\n",epsilon*step+starttime,a+currentshift*dx,dens_mom2/dens_mom0-a*a,fixedN);
    if(quiet < 1) {
      for(i=0;i<space;i++) {
...
void print_options() {
  if(quiet < 2) {
    fprintf(stdout,"# stochasitc simulations of adaptive waves...\n");
