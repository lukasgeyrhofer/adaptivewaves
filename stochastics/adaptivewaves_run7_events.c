...
int subpop_inject = 0;
double subpop_fixationthreshold = 1e-4;
double subpop_extinctionthreshold = 1e-4;
double subpop_labelthreshold = 0.;
double subpop_expectedfixationprobability = 0.5;

double subpop_fix_including;
double subpop_fix_excluding;
double subpop_current_fixationprob;

double subpop_last_fixationprob;
double subpop_crossoverpoint;
double subpop_crossover_step;
double subpop_crossover_scale = -1.;

int subpop_identicalstartpopulation = 1;
int subpop_labeltype = 0;
...
void parsecommandline(int argn, char* argv[]) {
  char c;
  int haveinfile = 0;
  while((c = getopt(argn,argv,"c:C:u:e:S:O:R:t:M:qQv:V:m:L:rH:h:")) != -1) {
    switch(c) {
      ...
      case 'L':	subpop_expectedfixationprobability = atof(optarg);
		subpop_active = 1;
		break;
      case 'H':	subpop_labeltype = atoi(optarg);
		subpop_active = 1;
		break;
      case 'h':	if(subpop_labeltype == 3) {
		  subpop_crossover_scale = atof(optarg);
		}else{
		  print_error("option -h only available for -H 3 (additional information for labelling algorithm -- crossover scale)");
		}
		break;
      case 'r':	subpop_identicalstartpopulation = 0;
		break;
    }
  }
...
int get_fitness_bin() {
  ...
  for(i=space-1;i>0;i--) {
    cumulative_g += u[i]*n[i];
    if(cumulative_g > subpop_expectedfixationprobability)break;
  }
  tmp = cumulative_g - u[i]*n[i];
  subpop_fix_including = cumulative_g;
  subpop_fix_excluding = tmp;
...
void reproduction_subpop() {
  ...
  for(i=1;i<space-1;i++) {
    nv_new[i] = coeff_m[i]*nv[i-1] + coeff_0[i]*nv[i] + coeff_p[i] * nv[i+1];
    nv_new[i] += twoepssqrt*(gsl_ran_poisson(rg,nv[i])-nv[i]);
    nw_new[i] = coeff_m[i]*nw[i-1] + coeff_0[i]*nw[i] + coeff_p[i] * nw[i+1];
...
double get_int_uc_subpop() {
  int i;
  double sum = 0.;
  subpop_current_fixationprob = 0.;
  for(i=0;i<space;i++) {
    sum+=u[i]*(nv[i]+nw[i]);
    subpop_current_fixationprob += u[i] * nw[i];
  }
  subpop_current_fixationprob /= sum;
  return sum;
}
...
void write_output(int step) {
  int i;
  double tmp_ftvar = dens_mom2/dens_mom0 - dens_mom1*dens_mom1/(dens_mom0*dens_mom0);
  if(quiet < 2) {
    fprintf(stdout,"%10.3lf %17.10e %.10e %.10e %.10e %.10e",epsilon*step,dens_mom1/dens_mom0,tmp_ftvar,dens_mom0);
    if(subpop_active == 1) {
      subpop_fraction = get_subpop_fraction();
      fprintf(stdout," %.10e %.10e",subpop_fraction*dens_mom0,subpop_current_fixationprob);
    }
    fprintf(stdout,"\n");
    if(quiet < 1) {
      for(i=0;i<space;i++) {
...
void relabel_population(int step, int ef) {
  ...
  if(subpop_identicalstartpopulation == 1) {
    ...
    for(i=0;i<space;i++)n[i] = nv[i] + nw[i];
...
void create_subpopulation() {
  ...
  if(subpop_active == 1) {
    ...
    switch(subpop_labeltype) {
      case 0:	label_startindex = get_fitness_bin();
		...
		tmp = (subpop_fix_including - subpop_expectedfixationprobability)/(subpop_fix_including - subpop_fix_excluding);
		nw[label_startindex] = n[label_startindex] * (1. - tmp);
		nv[label_startindex] = n[label_startindex] * tmp;
		
		subpop_current_fixationprob = subpop_expectedfixationprobability;
		break;
      case 1:	for(i=0;i<space;i++) {
		  nw[i] = subpop_expectedfixationprobability * n[i];
		  nv[i] = (1.-subpop_expectedfixationprobability) * n[i];
		}
		subpop_current_fixationprob = subpop_expectedfixationprobability;
		break;
      case 2:	label_startindex = 0;
		while(label_startindex < space) {
		  memcpy(&nv[0],&n[0],label_startindex*sizeof(double));
		  for(i=label_startindex;i<space;i++) {
		    nw[i] = n[i]*(i-label_startindex)/(space-label_startindex);
		    nv[i] = n[i] - nw[i];
		  }
		  printf("# %5d %.10e\n",label_startindex,get_subpop_fraction());
		  label_startindex++;
		}
		exit(1);
		break;
      case 3:	fprintf(stderr,"# finding crossoverpoint\n");
		j = 0;
		subpop_current_fixationprob = 0.;
		subpop_last_fixationprob = 0.;
		subpop_crossoverpoint = -space0*dx;
		subpop_crossover_step = dx;
		if(subpop_crossover_scale < 0)subpop_crossover_scale = 10*dx; // default value
		for(i=0;i<space;i++) {
		  nw[i] = cutoff_function((i-space0)*dx,subpop_crossoverpoint,subpop_crossover_scale)*n[i];
		  nv[i] = n[i] - nw[i];
		}
		subpop_current_fixationprob = get_subpop_fraction();
		subpop_last_fixationprob = subpop_current_fixationprob;
		while(fabs(subpop_current_fixationprob - subpop_expectedfixationprobability)>1e-10) {
		  subpop_crossoverpoint += subpop_crossover_step;
		  subpop_current_fixationprob = 0;
		  for(i=0;i<space;i++) {
		    nw[i] = cutoff_function((i-space0)*dx,subpop_crossoverpoint,subpop_crossover_scale)*n[i];
		    nv[i] = n[i] - nw[i];
		    subpop_current_fixationprob += nw[i]*u[i];
		  }
		  fprintf(stderr,"# %d %.10e %.10e %.10e\n",j++,subpop_current_fixationprob,subpop_crossover_step,subpop_crossoverpoint);
		  if( (subpop_current_fixationprob - subpop_expectedfixationprobability)*(subpop_last_fixationprob - subpop_expectedfixationprobability) < 0)subpop_crossover_step *= -.5;
		  subpop_last_fixationprob = subpop_current_fixationprob;
		}
		break;
...
//     printf("#sp_cf = %e\n",subpop_current_fixationprob);
    if(subpop_current_fixationprob < subpop_extinctionthreshold) {
      subpop_extinctions++;
      eventcount++;
      relabel_population(i,0);
    }
    if(1. - subpop_current_fixationprob < subpop_fixationthreshold) {
      subpop_fixations++;
      eventcount++;
