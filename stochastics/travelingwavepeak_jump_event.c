 ...
 * 
 * #################################################
 *  current implementation labels a subpopulation
 *  and tracks its dynamics
 * #################################################
 */
...
int initialize() {
  ...
  mutation_eps = epsilon*mutationrate;
...
void reproduce(int timestep) {
  ...
  for(i=1;i<space-1;i++) {
    ...
    tmpw[i] += mutation_eps*(ww[i-1]-ww[i]);
    tmpv[i] += mutation_eps*(vv[i-1]-vv[i]);
    if(tmpw[i]<0) {
      tmpw[i] = 0;
    }else if(noise > 0) {
