...
double *nv,*nv_new;
int subpop_inject = 0;
double subpop_fixationthreshold = 1e-2;
...
void parsecommandline(int argn, char* argv[]) {
  char c;
  int haveinfile = 0;
  while((c = getopt(argn,argv,"c:C:u:e:S:O:R:t:M:qQv:V:m:w:W:YL:")) != -1) {
    switch(c) {
      ...
      case 'L':	subpop_labelthreshold = atof(optarg);
		subpop_active = 1;
		subpop_label = 1;
		subpop_fromfile = 0;
		break;
      case 'w':	strcpy(winfilename,optarg);
		subpop_active = 1;
		subpop_fromfile = 1;
		subpop_label = 0;
		break;
      case 'W':	strcpy(woutfilename,optarg);
		subpop_haveoutfile = 1;
		break;
    }
  }
...
double get_int_uc_subpop() {
  int i;
  double sum = 0.;
  for(i=0;i<space;i++)sum+=u[i]*(nv[i]+nw[i]);
  return sum;
}
...
void write_output(int step) {
  int i;
  if(quiet < 2) {
    fprintf(stdout,"%10.4lf %16.3lf %16.10lf %16.10lf %16.10lf",epsilon*step,dens_mom0,dens_mom1/dens_mom0,dens_mom2/dens_mom0,lambda);
    if(subpop_active == 1) {
...
void relabel_population() {
  ...
  if(subpop_fromfile == 1) {
    read_w(0);
    for(i=0;i<space;i++) {
      if(nw[i] > n[i])n[i] = nw[i];
      nv[i] = n[i] - nw[i];
    }
  }
  
  if(subpop_label == 1) {
    label_startindex = (int)(subpop_labelthreshold/dx) + space0;
...
void create_subpopulation() {
  ...
  if(subpop_fromfile == 1) {
    ...
    for(i=1;i<space-1;i++)nv[i] = n[i] - nw[i];
  }
