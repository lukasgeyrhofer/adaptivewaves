...
void parsecomamndline(int argn, char *argv[]) {
  char c;
  while((c = getopt(argn,argv,"s:z:d:S:e:D:O:qQi:o:R:N:Cu:U:T:H:h:Pn:g:E:")) != -1) {
    switch(c) {
...
// 		break;
      ...
      case 'P':	printhistotype = 0;
		break;
      case 'E':	if(noise == 2) {
		  fe_expected_fixationprobability = atof(optarg);
		}else{
		  print_error("event based simulations only possible with tuned models, use option -u FILENAME");
		}
		break;
      }
  }
...
void reproduce(int timestep) {
  ...
  for(i=1;i<space-1;i++) {
    ...
    tmpw[i] += epsilon*mutationrate*ww[i-1];
    tmpv[i] += epsilon*mutationrate*vv[i-1];
    if(tmpw[i]<0) {
      tmpw[i] = 0;
    }else if(noise > 0) {
	...
	fprintf(stderr,"# reset bin, i=%d, step=%d\n",i,timestep);
	tmpw[i] = 1e9;
      }
    }
...
void write_fe() {
  ...
  if(fpc != NULL) {
    ...
    fwrite(&space,sizeof(int),1,fpc);
    fwrite(&space0,sizeof(int),1,fpc);
    for(i=0;i<space;i++)ww[i] /= dx;
    fwrite(ww,sizeof(double),space,fpc);
    fclose(fpc);
  }else{
    print_error("could not open c-outfile");
  }  
