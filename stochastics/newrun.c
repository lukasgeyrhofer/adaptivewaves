...
void parsecommandline(int argn, char* argv[]) {
  char c;
  int haveinfile = 0;
  while((c = getopt(argn,argv,"c:C:u:e:S:O:R:t:M:qQ")) != -1) {
    switch(c) {
...
void reproduction() {
  ...
  for(i=1;i<space-1;i++) {
    ...
    n_new[i] += twoepssqrt*(gsl_ran_poisson(rg,n_new[i])-n_new[i]); 
  }
  n_new[last] = 0.;
...
void initialize() {
  ...
  coeff_m[0] = 0.;
  coeff_0[0] = 1. - 2.*epsilon*diffusionconstant/(dx*dx) + (i-space0)*dx*epsilon - rho*epsilon;
  ...
  coeff_0[space-1] = 1. - 2.*epsilon*diffusionconstant/(dx*dx) + (i-space0)*dx*epsilon - rho*epsilon;
  coeff_p[space-1] = 0.;
...
void write_output(int step) {
  int i;
  if(quiet < 2) {
    fprintf(stderr,"%10.4lf %16.3lf %16.10lf %16.10lf\n",epsilon*step,dens_mom0,dens_mom1/dens_mom0,lambda);
    if(quiet < 1) {
      for(i=0;i<space;i++) {
